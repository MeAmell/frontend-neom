...
async function fetchFDR() {
  ...
  const res = await fetch(`${API_BASE}/api/fdr-progress`, {
...
// ─── Group raw fdrStages (flat per-event) → grouped per unique stage ──────────
function groupStagesByName(stages = []) {
  const STAGE_META = {
    'STAGE 0': { title: 'Persiapan & Migrasi Awal', phase: 'Pre-Cutover' },
    'STAGE 1': { title: 'Cutover Migration',        phase: 'Cutover'     },
    'STAGE 2': { title: 'T24 Upgrade & Konversi',   phase: 'Cutover'     },
    'STAGE 3': { title: 'Delta Migration & PTR',    phase: 'Cutover'     },
    'STAGE 4': { title: 'Go-Live & Post Deployment',phase: 'Post-Cutover'},
  ...
  for (const s of stages) {
    ...
    if (!map[key]) {
      const meta = STAGE_META[key] || { title: s.title || s.stage, phase: s.phase || 'Cutover' }
      map[key] = {
        stage: s.stage,
        title: s.title || meta.title,
        phase: s.phase || meta.phase,
        done: 0, in_progress: 0, not_started: 0, total: 0,
        done_pct: 0, in_progress_pct: 0, progress_pct: 0,
        ojk_status: 'NOT_STARTED',
        events: [],
      }
    }
    const g = map[key]
    g.done        += s.done        || 0
    g.in_progress += s.in_progress || 0
      ...
      if (s.event) {
        ...
        if (!existing) {
          g.events.push({
            ...
            total: s.event_total || 0,
            progress_pct: s.event_progress_pct || 0,
            done_pct: s.event_done_pct || 0,
            activity_list: s.activity_list || [],
          })
        }
      } else if (Array.isArray(s.events)) {
        g.events.push(...s.events)
      }
    }
  }
  ...
  return Object.values(map).map(g => {
    ...
    g.in_progress_pct = Math.round((g.in_progress / t) * 1000) / 10
    g.not_started_pct = Math.round((g.not_started / t) * 1000) / 10
    g.progress_pct    = g.done_pct
    g.ojk_status      = g.done_pct >= 100 ? 'DONE' : g.in_progress > 0 ? 'IN_PROGRESS' : 'NOT_STARTED'
    return g
  })
}
...
function computeOverallFromItems(items = []) {
  ...
  if (total === 0) return {}
  const done       = items.filter(r => (r.Status || '').toLowerCase().includes('done')).length
...
function StatusChip({ status }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: s.bg, color: s.color, borderRadius: '99px', padding: '3px 10px', fontSize: '11px', fontWeight: '700', whiteSpace: 'nowrap' }}>
...
function GanttTimeline() {
  ...
  const toX = (dateStr) => {
    ...
    return Math.max(0, Math.min(100, ((d - windowStart) / totalMs) * 100))
  }
  const todayX = toX(today.toISOString())
  const months = []
  for (let m = 0; m <= 5; m++) {
    const d = new Date(2026, m, 1)
    months.push({ label: d.toLocaleDateString('id-ID', { month: 'short' }), x: toX(d.toISOString()) })
  }
  const rows = [
  ...
  return (
    <div style={{ padding: '0 4px' }}>
      <div style={{ position: 'relative', height: '28px', marginBottom: '6px', marginLeft: '88px' }}>
        {months.map((m, i) => (
          <div key={i} style={{ position: 'absolute', left: `${m.x}%`, display: 'flex', flexDirection: 'column', alignItems: 'center', transform: 'translateX(-50%)' }}>
        ...
        <div style={{ position: 'absolute', left: `${todayX}%`, top: 0, bottom: 0, width: '2px', background: 'rgba(220,38,38,.5)', transform: 'translateX(-50%)' }} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {rows.map(row => (
          <div key={row.key} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '80px', flexShrink: 0, fontSize: '11px', fontWeight: '600', color: '#64748B', textAlign: 'right' }}>{row.label}</div>
            <div style={{ flex: 1, height: '28px', position: 'relative', background: '#F8FAFC', borderRadius: '6px' }}>
              {months.map((m, i) => <div key={i} style={{ position: 'absolute', left: `${m.x}%`, top: 0, bottom: 0, width: '1px', background: '#E2E8F0' }} />)}
              {row.bars.map((bar, bi) => {
                const x1 = toX(bar.start), x2 = toX(bar.end)
                ...
                if (bar.milestone) return (
                  <div key={bi} title={`${bar.label}: ${fmtDateShort(bar.start)}`} style={{ position: 'absolute', left: `${x1}%`, top: '50%', transform: 'translate(-50%, -50%)', width: '14px', height: '14px', background: bar.color, borderRadius: '3px', rotate: '45deg', boxShadow: `0 0 0 3px ${bar.color}30`, zIndex: 2 }} />
                )
                return (
                  <div key={bi} title={`${bar.label}: ${fmtDateShort(bar.start)} → ${fmtDateShort(bar.end)}`} style={{ position: 'absolute', left: `${x1}%`, width: `${w}%`, top: '6px', height: '16px', background: isPast ? `${bar.color}cc` : isActive ? bar.color : `${bar.color}55`, borderRadius: '4px', border: isActive ? `2px solid ${bar.color}` : 'none', boxShadow: isActive ? `0 2px 8px ${bar.color}40` : 'none', display: 'flex', alignItems: 'center', overflow: 'hidden', paddingLeft: '5px', cursor: 'default', zIndex: 1 }}>
                    {w > 8 && <span style={{ fontSize: '9px', fontWeight: '700', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden' }}>{bar.label}</span>}
                  </div>
                )
              })}
              <div style={{ position: 'absolute', left: `${todayX}%`, top: 0, bottom: 0, width: '2px', background: '#DC2626', transform: 'translateX(-50%)', zIndex: 3 }}>
      ...
      <div style={{ display: 'flex', gap: '16px', marginTop: '14px', marginLeft: '88px', flexWrap: 'wrap' }}>
        {[{ color: '#6366F1', label: 'Pre-Cutover' }, { color: '#0EA5E9', label: 'FDR' }, { color: '#E8A030', label: 'Cutover' }, { color: '#01847C', label: 'Post-Cutover' }, { color: '#DC2626', label: 'Go-Live ◆' }].map(l => (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {stages.map((stage, si) => {
        ...
        const progPct = stage.progress_pct || stage.done_pct || 0
        ...
        return (
          <div key={si} style={{ borderRadius: '12px', border: `1px solid ${isOpen ? '#01847C30' : '#E2E8F0'}`, overflow: 'hidden', boxShadow: isOpen ? '0 2px 12px rgba(1,132,124,.08)' : 'none' }}>
            ...
            <div onClick={() => setOpenStage(p => p === si ? null : si)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', cursor: 'pointer', background: isOpen ? '#F8FFFE' : '#fff', transition: 'background .15s' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: '800', color: progPct > 0 ? '#01847C' : '#94A3B8', whiteSpace: 'nowrap' }}>{progPct}%</span>
            ...
            {isOpen && (
              <div style={{ borderTop: '1px solid #F1F5F9', background: '#FAFCFF' }}>
                {(stage.events || []).length === 0 ? (
                  ...
                  const evProg  = ev.progress_pct || ev.done_pct || (evTotal > 0 ? Math.round((evDone / evTotal) * 100) : 0)
                  ...
                  return (
                    <div key={ei} style={{ borderBottom: ei < stage.events.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                      <div onClick={() => setOpenEvent(p => ({ ...p, [evKey]: !p[evKey] }))} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px 10px 44px', cursor: 'pointer', background: evOpen ? '#F1F5F9' : 'transparent', transition: 'background .12s' }}>
...
function FDRStageBar({ stage }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '14px', padding: '16px 20px', border: `1.5px solid ${iPct > 0 && cPct < 100 ? '#E8A03040' : '#F1F5F9'}`, boxShadow: '0 1px 4px rgba(0,0,0,.05)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ background: statusColor, color: '#fff', borderRadius: '8px', padding: '3px 10px', fontSize: '11px', fontWeight: '800' }}>{stage.stage}</div>
          {iPct > 0 && cPct < 100 && (
...
export default function DashboardPage({ user, onLogout }) {
  ...
  const loadOJK = useCallback(async () => {
    try { const d = await fetchDashboard(filterStage); setOjkData(d) }
  ...
  const computedOverall = (fdrItems.length > 0 && activeStageKey)
    ? computeOverallFromItems(filteredItems)
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ color: '#fff', fontFamily: 'monospace', fontSize: '12px' }}>
                {fdrData?.last_update ? new Date(fdrData.last_update).toLocaleString('id-ID') : ojkData?.last_update ? new Date(ojkData.last_update).toLocaleString('id-ID') : '—'}
              </div>
              <div style={{ marginTop: '4px', fontSize: '10px', color: 'rgba(255,255,255,.4)' }}>{fdrData ? '📊 Sumber: Excel FDR' : '📡 Sumber: Google Sheets'}</div>
            </div>
            {dGoLive !== null && (
              <div style={{ textAlign: 'right' }}>
                <div style={{ color: 'rgba(255,255,255,.5)', fontSize: '11px', fontWeight: '600', marginBottom: '4px' }}>GO-LIVE</div>
                <div style={{ background: 'rgba(220,38,38,.2)', color: '#FCA5A5', padding: '6px 16px', borderRadius: '99px', fontSize: '14px', fontWeight: '800', border: '1px solid rgba(220,38,38,.3)' }}>D-{Math.abs(dGoLive)}</div>
              </div>
            )}
            <div style={{ textAlign: 'right' }}>
      ...
      <main style={{ padding: '28px 32px', flex: 1, maxWidth: '1440px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap' }}>
          {[null, 0, 1, 2, 3, 4, 5].map(s => (
            <button key={s} onClick={() => setFilterStage(s)} style={{ padding: '6px 16px', borderRadius: '99px', border: 'none', fontWeight: '600', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', background: filterStage === s ? '#01847C' : '#fff', color: filterStage === s ? '#fff' : '#64748B', boxShadow: filterStage === s ? '0 2px 8px rgba(1,132,124,.3)' : '0 1px 3px rgba(0,0,0,.08)', transition: 'all .2s' }}>
          ...
          {[
            ...
            <div key={k.label} style={{ background: '#fff', borderRadius: '20px', padding: '28px', boxShadow: '0 1px 8px rgba(0,0,0,.06)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: `${k.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px' }}>{k.icon}</div>
              </div>
              <div style={{ marginTop: '20px' }}><ProgressBar value={parseFloat(k.value) || 0} color={k.color} /></div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: '20px', padding: '24px 28px', boxShadow: '0 1px 8px rgba(0,0,0,.06)', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            <div>
              ...
              {groupedFdrStages.length > 0 && <p style={{ fontSize: '11px', color: '#94A3B8', marginTop: '2px' }}>Data real-time dari Excel FDR · diperbarui otomatis saat file berubah</p>}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {stageBarData.length > 0
              ? stageBarData.map((s, i) => <FDRStageBar key={i} stage={s} />)
              : (activeStageKey ? groupedFdrStages.filter(s => (s.stage || '').toUpperCase() === activeStageKey) : groupedFdrStages).map((s, i) => <FDRStageBar key={i} stage={s} />)
            }
          </div>
        </div>
